...
class Args:
    ...
    """the device to run the experiment on (you may need to change to `cpu` if you don't have a GPU)"""
    seed: int = 1331
    """seed of the experiment"""
    save_model_frequency: int = 500000
    """save model freq"""
    ...
    num_steps: int = 2000
    """(changed)the number of steps to run in each environment per policy rollout"""
    anneal_lr: bool = True
    ...
    """(changed)the number of mini-batches"""
...
def make_env(env_id, task_id, seed, idx, capture_video, run_name, gamma):
    def thunk():
...
class Agent(nn.Module):
    def __init__(self, envs):
        super().__init__()
        self.attn = nn.Parameter(torch.zeros(np.array(envs.single_observation_space.shape).prod()))
...
if __name__ == "__main__":
    ...
    print("batch size:", args.batch_size, "num iterations:", args.num_iterations)
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, args.task_id, args.seed, i, args.capture_video, run_name, args.gamma) for i in range(args.num_envs)]
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            uncontrollable_distractors = np.random.uniform(low=-1, high=1, size=(1, num_uncontrollable_distractors)) + random_variable_bias
            controllable_distractors = np.matmul(action.detach().cpu().numpy(), control_weights) + controllable_variable_bias
            next_obs = np.concatenate((next_obs, controllable_distractors, uncontrollable_distractors), axis=1)
            ...
            if (global_step + 1) % args.save_model_frequency == 0:
                model_path = f"{run_name}/{global_step+1}_steps.cleanrl_model"
