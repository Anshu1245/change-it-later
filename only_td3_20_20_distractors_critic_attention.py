...
def save_checkpoint(shared_attn, actor, actor_target, qf1, qf2, qf1_target, qf2_target, q_optimizer, actor_optimizer, run_name, global_step, ckpt_path=None):
...
"""
    ...
    q_optimizer = optim.Adam(list(qf1.parameters()) + list(qf2.parameters()) + [shared_attn], lr=args.learning_rate)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:
            ...
            with torch.no_grad():
                actions = actor(torch.Tensor(obs).to(device), shared_attn.detach())
        ...
        if "final_info" in infos:
            ...
            while True:
                test_actions = actor(torch.Tensor(test_obs).to(device), shared_attn.detach())
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                ...
                next_state_actions = (target_actor(data.next_observations, shared_attn.detach()) + clipped_noise).clamp(
            ...
            if global_step % args.policy_frequency == 0:
                actor_loss = -qf1(data.observations, actor(data.observations, shared_attn.detach()), shared_attn).mean()
        ...
        if (global_step + 1) % args.save_model_frequency == 0:
                save_checkpoint(shared_attn, actor, target_actor, qf1, qf2, qf1_target, qf2_target, q_optimizer, actor_optimizer, run_name, global_step) #+"__"+str(global_step))
