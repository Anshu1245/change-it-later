...
def save_checkpoint(shared_attn, actor, qf1, qf2, qf1_target, qf2_target, q_optimizer, actor_optimizer, run_name, global_step, ckpt_path=None):
        if not os.path.exists('{}/'.format(run_name)):
            os.makedirs('{}/'.format(run_name))
        if ckpt_path is None:
            ckpt_path = "{}/{}_steps".format(run_name, global_step)
        print('Saving models to {}'.format(ckpt_path))
        torch.save({'shared_attention_parameter': shared_attn,
...
class SoftQNetwork(nn.Module):
    def __init__(self, env):
        super().__init__()
        # self.attn = nn.Parameter(torch.zeros(np.array(env.single_observation_space.shape).prod() + np.prod(env.single_action_space.shape)))
        # self.current_probs = torch.nn.functional.sigmoid(self.attn)
        self.fc1 = nn.Linear(np.array(env.single_observation_space.shape).prod() + np.prod(env.single_action_space.shape), 256)
        self.fc2 = nn.Linear(256, 256)
        self.fc3 = nn.Linear(256, 1)

    def forward(self, x, a, shared_attn):
        # self.current_probs = torch.nn.functional.sigmoid(self.attn)
        # x = x * self.current_probs
        # self.current_probs = torch.nn.functional.sigmoid(shared_attn)
        attn_wts = torch.nn.functional.sigmoid(shared_attn)
        x = x * attn_wts
        x = torch.cat([x, a], 1)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
...
class Actor(nn.Module):
    ...
    def forward(self, x, shared_attn):
        attn_wts = torch.nn.functional.sigmoid(shared_attn)
        x = x * attn_wts
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
    ...
    def get_action(self, x, shared_attn):
        mean, log_std = self(x, shared_attn)
        std = log_std.exp()
...
"""
    ...
    # shared_attn = torch.zeros(np.array(envs.single_observation_space.shape).prod(), requires_grad=True, device=device)
    shared_attn = torch.nn.Parameter(torch.zeros(np.array(envs.single_observation_space.shape).prod(), device=device), requires_grad=True)
    q_optimizer = optim.Adam(list(qf1.parameters()) + list(qf2.parameters()) + [shared_attn], lr=args.q_lr)
    ...
    # print("uncontrollable", uncontrollable_distractors.shape)
    # w = np.random.uniform(low=-1, high=1, size=(np.prod(envs.single_action_space.shape), num_uncontrollable_distractors))
    # w = np.random.normal(size=(np.prod(envs.single_action_space.shape), num_uncontrollable_distractors))
    # print("w", w.shape)
    ...
    # print("controllable", controllable_distractors.shape)
    # print("obs", obs.shape)
    obs = np.concatenate((obs, controllable_distractors, uncontrollable_distractors), axis=1)
    # print("obs new", obs.shape)

    if not os.path.exists('{}/'.format(run_name)):
        os.makedirs('{}/'.format(run_name))

    for global_step in range(args.total_timesteps):
        # alpha = (alpha_start - alpha_finish) * np.exp(-alpha_decay * global_step / args.total_timesteps) + alpha_finish                     
        # envs.render()
        ...
        if global_step < args.learning_starts:
            ...
            actions, _, _ = actor.get_action(torch.Tensor(obs).to(device), shared_attn.detach())
            if (global_step+1) % 10000 == 0:    
                print("step", global_step+1)
                print("shared critic attention wts", torch.nn.functional.sigmoid(shared_attn).detach().cpu().numpy())
            # print("qf1 attention wts", qf1.current_probs)
            # print("qf2 attention wts", qf2.current_probs)
        ...
        # w = np.random.uniform(low=-1, high=1, size=(np.prod(envs.single_action_space.shape), num_uncontrollable_distractors))
        # w = np.random.normal(size=(np.prod(envs.single_action_space.shape), num_uncontrollable_distractors))
        # controllable_distractors = np.matmul(actions, w)
        # print("actions, control weights", actions.shape, control_weights.shape)
        ...
        if "final_info" in infos:
            ...
            while True:
                _, _, test_actions = actor.get_action(torch.Tensor(test_obs).to(device), shared_attn.detach())
        ...
        if global_step > args.learning_starts:
            ...
            # current_attn = torch.nn.functional.sigmoid(actor.attn).detach()
            # print("current attn 1", torch.nn.functional.sigmoid(actor.attn))
            with torch.no_grad():
                next_state_actions, next_state_log_pi, _ = actor.get_action(data.next_observations, shared_attn.detach())
                qf1_next_target = qf1_target(data.next_observations, next_state_actions, shared_attn.detach())
                qf2_next_target = qf2_target(data.next_observations, next_state_actions, shared_attn.detach())
                min_qf_next_target = torch.min(qf1_next_target, qf2_next_target) - alpha * next_state_log_pi
                next_q_value = data.rewards.flatten() + (1 - data.dones.flatten()) * args.gamma * (min_qf_next_target).view(-1)

            qf1_a_values = qf1(data.observations, data.actions, shared_attn).view(-1)
            qf2_a_values = qf2(data.observations, data.actions, shared_attn).view(-1)
            ...
            q_optimizer.step()
            # print(torch.equal(current_attn, torch.nn.functional.sigmoid(actor.attn)))

            if global_step % args.policy_frequency == 0:  # TD 3 Delayed update support
                for _ in range(
                    args.policy_frequency
                ):  # compensate for the delay by doing 'actor_update_interval' instead of 1
                    pi, log_pi, _ = actor.get_action(data.observations, shared_attn.detach())
                    # print("current attn 3", torch.nn.functional.sigmoid(actor.attn))
                    qf1_pi = qf1(data.observations, pi, shared_attn)
                    qf2_pi = qf2(data.observations, pi, shared_attn)
                    ...
                    if args.autotune:
                        with torch.no_grad():
                            _, log_pi, _ = actor.get_action(data.observations, shared_attn.detach())
        ...
        if (global_step + 1) % args.save_model_frequency == 0:
            save_checkpoint(shared_attn, actor, qf1, qf2, qf1_target, qf2_target, q_optimizer, actor_optimizer, run_name, global_step) #+"__"+str(global_step))
