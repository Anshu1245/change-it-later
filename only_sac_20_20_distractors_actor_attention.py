...
class Actor(nn.Module):
    def __init__(self, env):
        super().__init__()
        self.attn = nn.Parameter(torch.zeros(np.array(env.single_observation_space.shape).prod()))
        self.current_probs = torch.nn.functional.sigmoid(self.attn)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:
            ...
            if (global_step+1) % 10000 == 0:    
                current_attn = torch.nn.functional.sigmoid(actor.attn).detach().cpu().numpy()
                print("step", global_step+1)
                print("actor attention wts", current_attn)
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                ...
                qf1_next_target = qf1_target(data.next_observations, next_state_actions, current_attn)
                qf2_next_target = qf2_target(data.next_observations, next_state_actions, current_attn)
                min_qf_next_target = torch.min(qf1_next_target, qf2_next_target) - alpha * next_state_log_pi
                next_q_value = data.rewards.flatten() + (1 - data.dones.flatten()) * args.gamma * (min_qf_next_target).view(-1)

            qf1_a_values = qf1(data.observations, data.actions, current_attn).view(-1)
            qf2_a_values = qf2(data.observations, data.actions, current_attn).view(-1)
            ...
            if global_step % args.policy_frequency == 0:  # TD 3 Delayed update support
                for _ in range(
                    ...
                    qf1_pi = qf1(data.observations, pi, current_attn)
                    qf2_pi = qf2(data.observations, pi, current_attn)
